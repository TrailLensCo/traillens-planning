...
This document provides a comprehensive implementation plan for **TrailLensHQ MVP v1.13**, targeting a **Q2 2026 launch** with pilot organizations Hydrocut and GORBA (4 trail systems total).

**Project Scope:**
- **14 Implementation Phases**: From brand messaging to testing and validation
- **Target Launch**: Q2 2026 (April-June 2026)
- **Pilot Organizations**: Hydrocut (Glasgow + Synders) and GORBA (Guelph Lake + Akell)
...
**IMPORTANT DATA MODEL:** TrailLensHQ manages **trail systems** (collections of trails managed as one unit), NOT individual trails within those systems. Each organization has multiple trail systems (e.g., Hydrocut has Glasgow and Synders trail systems, GORBA has Guelph Lake and Akell trail systems).
...
- Three authentication methods (ALL REQUIRED):
  - Passkey (WebAuthn/FIDO2): Touch ID, Face ID, security keys
  - Magic Link: 15-minute expiration email links
  - Email/Password: 12+ char minimum with complexity requirements
- MFA enforcement for admin roles (7-day grace period)
...
**Pilot Metrics:**
- All 4 trail systems operational
...
**Needs Updates For MVP:**
- **Security Hardening**: Enable CloudTrail (1-year retention), deploy AWS WAF, enable Security Hub and GuardDuty
...
**Objective**: Update all public-facing content with new brand byline and messaging
...
**Phase 1 Total Duration**: 1-2 days
...
**Objective**: Address 7 critical security gaps to make platform production-ready
...
6. Enable Security Hub and GuardDuty
7. Implement MFA enforcement for admin roles
...
### Task 2.6: Enable Security Hub and GuardDuty

**Objective**: Enable continuous security monitoring and threat detection across AWS account

**Files to Modify**:
- `infra/components/security.py` (new file for Security Hub and GuardDuty)
- `infra/index.ts` or `__main__.py` (import security stack)

**Implementation Steps for Security Hub**:
1. Enable AWS Security Hub in ca-central-1
2. Enable security standards (CIS AWS Foundations Benchmark)
3. Configure automated findings aggregation
4. Set up SNS notifications for critical findings
5. Review initial findings and remediate critical issues

**Implementation Steps for GuardDuty**:
1. Enable AWS GuardDuty in ca-central-1
2. Configure finding export to S3 (for long-term storage)
3. Set up CloudWatch Event Rule for high-severity findings
4. Configure SNS notifications for critical threats
5. Review sample findings and tune alert thresholds

**Cost Impact**:
- GuardDuty: ~$4.40/month (10GB CloudTrail, 1GB VPC Flow Logs)
- Security Hub: ~$50/month ($0.0010 per check * 50,000 checks)

**Testing**:
- Generate test GuardDuty finding (use sample events)
- Verify SNS notification received
- Review Security Hub compliance dashboard
- Confirm findings appear in console

**Acceptance Criteria**:
- Security Hub enabled with CIS benchmark
- GuardDuty enabled with alerts configured
...
- `api-dynamo/middleware/auth.py` (MFA enforcement logic)
- `web/src/pages/Login.jsx` (MFA setup UI)

**Implementation Steps**:
1. Configure Cognito User Pool MFA settings:
   - MFA configuration: OPTIONAL (not ON, because regular users don't need MFA)
   - Allowed MFA methods: SMS and TOTP authenticator apps
2. Implement API middleware to check MFA status:
   - On first admin login, set `mfa_grace_period_end = current_time + 7 days`
   - On subsequent logins, if user is in admin group AND MFA not enabled AND grace period expired, force MFA setup
...
**Phase 2 Total Duration**: 5-7 days
**Phase 2 Success Criteria**:
- All 7 critical security gaps addressed
- CloudTrail, WAF, Security Hub, GuardDuty enabled
...
**Three Authentication Methods (ALL REQUIRED)**:
1. **Passkey Authentication**: WebAuthn/FIDO2 (Touch ID, Face ID, security keys)
2. **Magic Link**: Email-based passwordless login (15-minute expiration)
3. **Email/Password**: Traditional authentication (12+ char, complexity, 6-password history)

---

### Task 3.1: Research Cognito Passkey Support

**Objective**: Determine if AWS Cognito natively supports passkeys, or if custom implementation is needed

**Research Steps**:
1. Check AWS Cognito documentation for WebAuthn/FIDO2 support
2. Search for Cognito custom authentication flows with passkeys
3. Evaluate third-party libraries (e.g., SimpleWebAuthn)
4. Determine implementation approach:
   - Option A: Cognito native support (if available)
   - Option B: Custom authentication flow with Lambda trigger
   - Option C: Store passkey credentials in DynamoDB, verify in Lambda

**Decision Point**: Choose implementation approach based on research

**Acceptance Criteria**:
- Implementation approach documented
- Technical feasibility confirmed
- Libraries/dependencies identified
...
**Objective**: Enable biometric login using Touch ID, Face ID, or hardware security keys

**Files to Modify**:
- `api-dynamo/auth/passkey.py` (new file for passkey credential management)
- `api-dynamo/routes/auth.py` (passkey registration and authentication endpoints)
- `web/src/pages/Login.jsx` (passkey login button)
- `web/src/components/PasskeySetup.jsx` (new component for passkey registration)
- DynamoDB table: `passkey_credentials` (new table for storing public keys)

**Implementation Steps**:

**Backend (API)**:
1. Create DynamoDB table `passkey_credentials`:
   - PK: user_id
   - SK: credential_id
   - Attributes: public_key, counter, created_at, last_used
2. Implement `/auth/passkey/register-begin` endpoint:
   - Generate challenge (random bytes)
   - Store challenge in session or DynamoDB with expiration
   - Return challenge options (RP ID, user info, supported algorithms)
3. Implement `/auth/passkey/register-finish` endpoint:
   - Verify attestation response
   - Extract and store public key in DynamoDB
   - Link credential to user account
4. Implement `/auth/passkey/login-begin` endpoint:
   - Generate challenge
   - Return challenge and allowed credential IDs for user
5. Implement `/auth/passkey/login-finish` endpoint:
   - Verify assertion response using stored public key
   - Increment counter (replay attack prevention)
   - Issue Cognito JWT token
   - Return access token and refresh token

**Frontend (Web)**:
1. Add "Sign in with Passkey" button to login page
2. Implement passkey registration flow:
   - Call `/auth/passkey/register-begin`
   - Invoke `navigator.credentials.create()` with challenge
   - Call `/auth/passkey/register-finish` with attestation
   - Show success message
3. Implement passkey login flow:
   - Call `/auth/passkey/login-begin`
   - Invoke `navigator.credentials.get()` with challenge
   - Call `/auth/passkey/login-finish` with assertion
   - Store tokens in localStorage
   - Redirect to dashboard

**Testing**:
- Register passkey with Touch ID on macOS
- Login with passkey (verify biometric prompt)
- Test with hardware security key (YubiKey if available)
- Test error handling (user cancels, passkey not supported)
- Verify counter increments on each login
- Test passkey deletion and re-registration

**Acceptance Criteria**:
- Passkey registration working on supported devices
- Passkey login functional with biometrics
- Public keys securely stored in DynamoDB
- Replay attack prevention (counter check)
- Error handling graceful
- Documentation updated with browser support matrix

**AI-Assisted Timeline**: 12-16 hours

---

### Task 3.3: Implement Magic Link Authentication

**Objective**: Enable email-based passwordless login with 15-minute expiration links
...
**DATA MODEL CHANGE**: This is a CRITICAL architectural change. The platform currently manages individual trails, but MVP requires managing trail systems (collections of trails). Example: Hydrocut organization has two trail systems: "Glasgow" and "Synders".  Each trail system can contain multiple physical trails, but status is managed at the system level, not per-trail.
...
trail_systems = Table(
    ...
    attributes={
        ...
        "name": str,                 # e.g., "Glasgow Trail System"
...
5. Add seed data for Hydrocut and GORBA trail systems
...
- Trail System 1: "Glasgow Trail System"
  - Location: Kitchener, ON
  - Status: Open
  - Description: "Premier mountain biking trails..."
- Trail System 2: "Synders Trail System"
  - Location: Waterloo, ON
  - Status: Open
  - Description: "Family-friendly trails..."
...
- Pilot data seeded (4 trail systems)
- Tests passing
...
**Offline Queue Logic**:
1. When offline, save report to local Core Data or UserDefaults
...
**Caching Logic**:
1. Cache trail system data in Core Data or UserDefaults
...
### Task 13.1: Create Hydrocut Organization and 2 Trail Systems
...
3. Create 2 trail systems:
   - Glasgow Trail System (Kitchener, ON)
   - Synders Trail System (Waterloo, ON)
4. Upload cover photos for each trail system
...
- All 4 trail systems operational (Hydrocut: 2, GORBA: 2)
...
**Tests to Perform**:
1. AWS Security Hub compliance check
2. GuardDuty findings review
3. CloudTrail log verification
4. Penetration testing (manual or automated)
5. OWASP Top 10 vulnerability scan
6. SQL injection testing
7. XSS testing
8. CSRF protection verification
9. Authorization bypass attempts
10. Secrets scanning (no credentials in code)
...
1. **Phase 12 (iPhone Apps) - 25-40 days**
   - **Risk**: Longest phase, blocks pilot launch
...
2. **Phase 3 (Authentication) - 7-10 days**
   - **Risk**: Passkey implementation complexity
   - **Mitigation**: Research Cognito passkey support early, have fallback to magic link + email/password if passkey blocked
   - **Contingency**: Implement passkey post-MVP if Cognito doesn't support
...
| M2        | Feb 2, 2026  | Security Hardening Complete                                                 | CloudTrail (1-year), WAF, secrets rotation (180-day), incident response plan, Security Hub enabled |
| M3        | Feb 9, 2026  | Authentication System Complete                                              | Passkey + magic link + email/password working, MFA enforced for admins                             |
...
| M11       | Apr 23, 2026 | Pilot Organizations Live                                                    | Hydrocut + GORBA onboarded, 4 trail systems operational, admins trained                            |
...
- [ ] Hydrocut Trail System onboarded (4 trail systems: Hydrocut, Beaver Pond, Sassafras, Boulder Loop)
...
- **Minimum Pilot Organizations**: 2 organizations (Hydrocut Trail System, GORBA)
- **Minimum Trail Systems**: 4 trail systems (Hydrocut, Beaver Pond, Sassafras, Boulder Loop)
