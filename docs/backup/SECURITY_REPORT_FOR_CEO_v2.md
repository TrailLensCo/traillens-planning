...
✅ **PII Storage**: We store substantial PII (emails, names, phone numbers, GPS coordinates, device identifiers) across 16 DynamoDB tables
...
TrailLensHQ stores PII across **16 DynamoDB tables** in AWS Canada Central (ca-central-1). Based on comprehensive codebase analysis, here is the complete PII inventory:
...
| **User IDs** | Present in 14 of 16 tables | 10,000+ users | **MEDIUM** |
...
- DynamoDB tables (all 16 tables, all PII)
...
1. **Main API Lambda** (api-dynamo)
   - **Access:** All 16 DynamoDB tables (full read/write)
...
- **AWS Cognito** with strong password policy (8+ chars, mixed case, numbers, symbols)
...
#### ✅ Encryption
- **DynamoDB:** All 16 tables encrypted at rest (AES-256, AWS managed keys)
...
- **Recommendation:** Enable Secrets Manager auto-rotation (90-day cycle)
...
- **No Security Hub** - No continuous compliance monitoring
- **No GuardDuty** - No threat detection for AWS account
...
- **Recommendation:** Enable Security Hub ($0.0010/check) and GuardDuty ($4.40/month)
...
| Monitoring | 4/10 | Basic CloudWatch, but no Security Hub/GuardDuty |
...
- **Customers:** North American customers, small EU customers OK with Canadian storage
...
- **Architecture:** Full multi-region with failover
- **Target:** Global scalability and high availability
...
- Attacker exports all 16 tables to S3, downloads locally
...
- ⚠️ Enable GuardDuty (anomalous behavior detection)
...
- **Action:** Enable AWS CloudTrail with 90-day retention
...
#### **11. Enable Security Hub & GuardDuty (1 Hour)**
...
- **Cost:** GuardDuty $4.40/month, Security Hub $0.0010/check
- **Priority:** **HIGH**
...
| AWS GuardDuty | $4.40/mo | $4.40/mo | Threat detection |
| AWS Security Hub | $50/mo | $50/mo | Compliance monitoring |
...
| **Subtotal Infrastructure** | **$1.5K-2K** | **$1.5K-2K** | |
| | | | |
...
11. ✅ Enable Security Hub & GuardDuty ($55/month)
