...
- A **trail system** contains many individual trails (e.g., Glasgow trail system has multiple trails)
- Individual trails in a trail system are OUT OF SCOPE - we only manage trail systems
- Each **trail organization** has multiple **trail systems**:
  - **Hydrocut** → Glasgow trail system, Synders trail system
...
- **Pilot trail systems** - 4 total trail systems (Hydrocut: Glasgow + Synders, GORBA: Guelph Lake + Akell)
...
- Examples: Glasgow trail system (Hydrocut), Synders trail system (Hydrocut), Guelph Lake trail system (GORBA), Akell trail system (GORBA)
...
- **Name** - Trail system name (e.g., "Glasgow", "Synders")
...
- **Scheduled status changes** - Pre-schedule multiple future status changes per trail system
  - Example: Glasgow scheduled to "Close Nov 1", "Reopen Apr 1", "Close for maintenance Feb 15"
...
- **Individual trail systems** - Subscribe to specific trail systems (e.g., Glasgow only, not Synders)
...
- **Scheduled change reminders** - Notification before scheduled status change (e.g., "Glasgow closing Nov 1 in 3 days")
...
- **Trail Systems:**
  - **Glasgow** trail system (mountain biking focus)
  - **Synders** trail system (mountain biking focus)
...
- **Onboarding:** Both organizations fully set up within 2 weeks
  - All 4 trail systems created (Glasgow, Synders, Guelph Lake, Akell)
...
**GAP 7: Enable Security Hub and GuardDuty**
...
- **Features:**
  - **View trail systems** - Grouped by organization (Hydrocut → Glasgow, Synders)
...
5. Schedule future status changes (e.g., "Close Glasgow Nov 1 for winter")
...
1. Create and manage trail systems for my organization (add Glasgow, Synders, etc.)
...
✅ **Pilot onboarding** (Hydrocut with 2 trail systems, GORBA with 2 trail systems)
...
9. **UI display:** ✅ **DECISION: Grouped by organization** (show "Hydrocut" with nested trail systems Glasgow, Synders)
10. **Status history visibility:** ✅ **DECISION: Show history publicly** (display "Glasgow was Open, changed to Closed on Jan 14 by Jane Doe")
...
1. **Historical trail system data:** ✅ **DECISION: Start fresh with current trail systems only** (clean slate, create 4 trail systems from scratch: Glasgow, Synders, Guelph Lake, Akell)
...
3. **Create a comprehensive TODO list** using the TodoWrite tool with two main sections:
   ...
   **Recommended Implementation Phases** (for project plan reference):
   ...
   - **Phase 2:** Security hardening (CloudTrail, WAF, secrets rotation, incident response plan, Security Hub, GuardDuty)
   ...
   - **Phase 13:** Pilot onboarding (Hydrocut with Glasgow + Synders trail systems, GORBA with Guelph Lake + Akell trail systems, white-glove setup, live training)
...
- [ ] All security gaps identified (CloudTrail, WAF, secrets rotation, incident response, Security Hub, GuardDuty, MFA)
...
- [ ] 4 pilot trail systems identified (Glasgow, Synders, Guelph Lake, Akell)
...
| 1.3 | 2026-01-15 | CTO + Claude Code | CRITICAL DATA MODEL CHANGE: Replaced "trails" with "trail systems" throughout. MVP manages trail systems (Glasgow, Synders, Guelph Lake, Akell), NOT individual trails. Added trail system attributes, subscription model, 11 new data model decisions. 11 implementation phases (was 9). |
